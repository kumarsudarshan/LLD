...
public class SnakeAndLadderService {
    ...
    public void setSnakes(List<Snake> snakes) throws InvalidSnakePosition {
        for (Snake snake : snakes) {
            if (snake.getStart() >= snakeAndLadderBoard.getSize()
                    || snake.getEnd() >= snakeAndLadderBoard.getSize()
                    || snake.getStart() < 0
                    || snake.getEnd() <= 0) {
                throw new InvalidSnakePosition("Invalid snake position.");
            }
        }
        snakeAndLadderBoard.setSnakes(snakes); // Add snakes to board
    }

    public void setLadders(List<Ladder> ladders) throws InvalidLadderPosition {
        for (Ladder ladder : ladders) {
            if (ladder.getStart() >= snakeAndLadderBoard.getSize()
                    || ladder.getEnd() > snakeAndLadderBoard.getSize()
                    || ladder.getStart() < 0
                    || ladder.getEnd() <= 0) {
                throw new InvalidLadderPosition("Invalid ladder position.");
            }
        }
    ...
    private int getTotalValueAfterDiceRolls() {
        // if two consecutive dice roll 6 is allowed, or we can customized here too
        if (this.shouldAllowMultipleDiceRollOnSix) {
            boolean tryRollingDice = true;
            int rollValue = 0;
            while (tryRollingDice) {
                rollValue += DiceService.roll();
                if (rollValue == 18) {
                    rollValue = 0;
                    continue;
                }
                ...
                tryRollingDice = false;
            }
            return rollValue;
        }

        // if we have n number of dices.
        int totalRollSum = 0;
        for (int i = 0; i < noOfDices; i++) {
            totalRollSum += DiceService.roll();
        }
        return totalRollSum;
    }
