...
import parkinglot.model.parking.spot.CarParkingSpot;
import parkinglot.model.parking.spot.MotorBikeParkingSpot;
...
public class ParkingLotApplication {
    public static void main(String[] args) throws InvlaidParkingFloorException {
        ...
        parkingLotService.setAddress(address);
        //Admin tests
        Account adminAccount = new AdminService();
        //Admin Case 1 - should be able to add parking floor case
        ((AdminService) adminAccount).addParkingFloor(new ParkingFloor("1"));
        //Admin Case 2 - should be able to add parking floor case
        ((AdminService) adminAccount).addParkingFloor(new ParkingFloor("2"));

        //Admin Case 3 - should be able to add entrance panel
        EntrancePanel entrancePanel = new EntrancePanel("1");
        ((AdminService) adminAccount).addEntrancePanel(entrancePanel);

        //Admin Case 4 - should be able to add exit panel
        ExitPanel exitPanel = new ExitPanel("1");
        ((AdminService) adminAccount).addExitPanel(exitPanel);

        String floorId = parkingLotService.getParkingFloors().get(0).getFloorId();

        ///Admin case 5 - should be able to add car parking spot
        ParkingSpot carSpot1 = new CarParkingSpot("c1");
        ((AdminService) adminAccount).addParkingSpot(floorId, carSpot1);
        ///Admin case 6 - should be able to add bike parking spot
        ParkingSpot bikeSport = new MotorBikeParkingSpot("b1");
        ((AdminService) adminAccount).addParkingSpot(floorId, bikeSport);
        ///Admin case 7 - should be able to add car parking spot
        ParkingSpot carSpot2 = new CarParkingSpot("c2");
        ((AdminService) adminAccount).addParkingSpot(floorId, carSpot2);

        // Test case 1 - check for availability of parking lot - TRUE
        System.out.println(ParkingLotService.getInstance().canPark(VehicleType.CAR));

        // Test case 2 - check for availability of parking lot - FALSE
        System.out.println(ParkingLotService.getInstance().canPark(VehicleType.EBIKE));

        // Test case 3 - check for availability of parking lot - FALSE
        System.out.println(ParkingLotService.getInstance().canPark(VehicleType.ELECTRIC));

        // TEST case 4 - Check if full
        System.out.println(ParkingLotService.getInstance().isFull());

        // Test case 5 - get parking spot
        Vehicle vehicle = new Car("KA05MR2311");
        ParkingSpot availableSpot = ParkingLotService.getInstance().getParkingSpot(vehicle.getType());
        ...
        ParkingSpot vanSpot = ParkingLotService.getInstance().getParkingSpot(van.getType());
        System.out.println(null == vanSpot);

        //Test case 7 - Entrance Panel - 1
        System.out.println(ParkingLotService.getInstance().getEntrancePanels().size());
        ...
        System.out.println(ParkingLotService.getInstance().getParkingFloors()
